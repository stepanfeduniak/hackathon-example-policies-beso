...
from example_policies.policies.BESO.configuration_beso import BesoConfig
...
class BesoModel(nn.Module):
    def __init__(self, config: BesoConfig):
        ...
        self.dit_backbone = Noise_Dec_only(
            ...
            embed_pdrob=config.embed_pdrob,
            goal_seq_len=0,
            obs_seq_len=self.config.n_obs_steps,
            action_seq_len=self.config.n_action_steps,
            linear_output=False,
            use_ada_conditioning=False,
            diffusion_type="beso",  # ddpm, beso or rf,
            use_pos_emb=True
        )
        # self.dit_backbone = BesoConditionalUnet1d(config, global_cond_dim=global_cond_dim * config.n_obs_steps)
    ...
    def conditional_sample(
        ...
        sigmas = get_sigmas_exponential(self.sampling_steps, self.sigma_min, self.sigma_max, self.device)
        x = torch.randn((len(global_cond), self.horizon, self.action_dim), device=self.device) * self.sigma_max
        actions = sample_ddim(self.dit_backbone, x, input_state, None, sigmas)
        return actions
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        sigmas= make_sample_density('loglogistic',self.sigma_max,self.sigma_min)(shape=(len(trajectory),), device=self.device).to(self.device)
        #->
        ...
        model_output = self.dit_backbone(noised_input * c_in, sigmas, global_cond=global_cond)
