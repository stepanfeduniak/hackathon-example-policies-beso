...
class Noise_Dec_only(nn.Module):
    ...
    def forward(
        ...
        #print(f"Input shape: {states.shape}") # Debugging line
        #print(f"expected obs_seq_len: {self.obs_seq_len}") # Debugging line # why is it giving obs_Seq_len as 2 ?
        #print(f"actions shape: {actions.shape}") # Debugging line
        #assert t == self.obs_seq_len + 2 # + 2 is for two tactile tokens ?
        ...
        action_embed += self.pos_emb[:, self.goal_seq_len : (self.goal_seq_len + t_a), :]
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        print(f"DEBUG: TransformerEncoder initialized with {n_layers} layers, embed_dim={embed_dim}, n_heads={n_heads}, causal={causal}, use_cross_attention={use_cross_attention}")
